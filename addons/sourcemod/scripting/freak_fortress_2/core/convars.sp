...
void ConVar_PluginStart()
{
	...
	Cvar[DisguiseModels] = CreateConVar("ff2_game_disguises", "0", "If to use rome vision to apply custom models to disguises.", _, true, 0.0, true, 1.0);
	...
	Cvar[RaidLimit] = CreateConVar("ff2_game_raids_map", "1", "Maximum amount of raid bosses that can be selected during the map duration, 0 for no limit", _, true, 0.0);
	Cvar[RaidPlayers] = CreateConVar("ff2_game_raids_players", "16", "Minimum amount of playing players before a raid boss can be selected", _, true, 0.0);
